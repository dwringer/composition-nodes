...
def flatten_histogram(image_array, seed=None):
    ...
    if isinstance(image_array.flat[0], numpy.uint8):
        image_array[pixels[:, 1].astype('int'), 0] = (
            255. * pixels[:,2].astype('float') / (pixel_count - 1.0)
        ).astype('uint8')
    else:
        image_array[pixels[:, 1].astype('int'), 0] = (
            pixels[:, 2] / (pixel_count - 1.0)
        )
...
def white_noise_array(width, height, seed=None, is_uint8=True):
    ...
    if is_uint8:
        return (prng.random((height, width, 1)).reshape((height, width)) * 255.).astype('uint8')
    else:
        return (prng.uniform(0., 1., size=(height, width)))
...
def red_noise_array(width, height, iterations=5, sigma=0.5, radius=None, blur_threshold=0.005, seed=None, is_uint8=True):
...
def red_noise_image(width, height, iterations=5, sigma=0.5, radius=None, blur_threshold=0.005, seed=None):
    return PIL.Image.fromarray(red_noise_array(width, height, iterations, sigma, radius, blur_threshold, seed=seed), mode="L")
...
def blue_noise_image(width, height, iterations=5, sigma=0.5, radius=None, blur_threshold=0.005, seed=None):
    return PIL.Image.fromarray(blue_noise_array(width, height, iterations, sigma, radius, blur_threshold, seed=seed), mode="L")
...
def green_noise_array(
    ...
    if radius_strong is None:
        radius_strong = int(math.floor(1. + 2. * math.sqrt(-2. * sigma_strong * sigma_strong * math.log(blur_threshold)))) + 1
    if radius_weak is None:
        radius_weak =   int(math.floor(1. + 2. * math.sqrt(-2. * sigma_weak   * sigma_weak   * math.log(blur_threshold)))) + 1
    arr = white_noise_array(width, height, seed=seed, is_uint8=is_uint8)
    ...
    for i in range(iterations):
        ...
        arr = b_weak[radius_weak:-radius_weak, radius_weak:-radius_weak].copy() -  \
            b_strong[radius_strong:-radius_strong, radius_strong:-radius_strong]
        arr = flatten_histogram(arr,seed=seed)
    return arr

def green_noise_image(
        width, height, iterations=5, sigma_strong=2.0, sigma_weak=0.5, radius_strong=None, radius_weak=None, blur_threshold=0.005, seed=None
):
    return PIL.Image.fromarray(
        (green_noise_array(
            width, height, iterations, sigma_strong, sigma_weak, radius_strong, radius_weak, blur_threshold, seed=seed, is_uint8=False
